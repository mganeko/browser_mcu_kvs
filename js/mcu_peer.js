...
function prepareNewConnection(id) {
  ...
  peer.onconnectionstatechange = function () {
    ...
    if (peer.connectionState === 'failed') {
      console.log('-- connection failed, so give up --');
      console.log('dissconect only this peer id:' + id);
      if (_disconnectOneFunc) {
        _disconnectOneFunc(id);
      }
...
let _sendOfferFunc = null;
let _sendAnswerFunc = null;
let _sendIceCandidateFunc = null;

function setSendJsonFunc(func) {
  _sendJsonFunc = func;
}

function setSendOfferFunc(func) {
  _sendOfferFunc = func;
}

function setSendAnswerFunc(func) {
  _sendAnswerFunc = func;
}

function setSendIceCandidateFunc(func) {
  _sendIceCandidateFunc = func;
}
...
function sendSdp(id, sessionDescription) {
  if (sessionDescription.type === 'offer') {
    if (_sendOfferFunc) {
      ...
      _sendOfferFunc(id, sessionDescription);
      return;
    }
  }
    ...
    if (_sendAnswerFunc) {
      ...
      _sendAnswerFunc(id, sessionDescription);
      return;
    }
  }

  console.log('---sending sdp as JSON string ---');
...
function sendIceCandidate(id, candidate) {
  if (_sendIceCandidateFunc) {
    console.log('--send ICE candidate ---');
    _sendIceCandidateFunc(id, candidate);
    return;
  }

  console.log('---sending ICE candidate as JSON string ---');
